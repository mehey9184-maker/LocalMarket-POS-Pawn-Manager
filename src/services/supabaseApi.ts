...
export const shopItemsApi = {
  ...
  mapRowToInventoryItem(row: ShopItemRow): InventoryItem {
    return {
      ...
      acquisitionType: (row.acquisition_type || 'Existing Stock') as any,
      costBasis: row.cost_basis === null || row.cost_basis === undefined ? undefined : Number(row.cost_basis),
      ...
      sourceStatus: (row as any).source_status || undefined,
      sourceNote: (row as any).source_note || undefined,
      internalNote: (row as any).internal_note || undefined,
    };
  },
  ...
  mapInventoryItemToRow(item: InventoryItem, shopId?: string): Database['public']['Tables']['shop_items']['Insert'] {
    return {
      ...
      internal_note: item.internalNote || null,
...
export const sellersApi = {
  ...
  mapSellerToRow(seller: Seller, shopId?: string): Database['public']['Tables']['sellers']['Insert'] {
    return {
      ...
      verification_status: seller.verified ? 'verified' : 'pending',
