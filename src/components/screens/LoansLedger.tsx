...
export const LoansLedger: React.FC = () => {
  ...
  // Progressive Counter Settlement Drawer State
  ...
  }, [pawnLoans, searchQuery, filterTab, activeCustomer, filterOnlyActiveCustomer]);

  // Counts for pills
  const counts = useMemo(() => {
    return {
      all: pawnLoans.filter(l => l.status === 'Active').length,
      expiring: pawnLoans.filter(l => l.status === 'Active' && l.daysRemaining > 0 && l.daysRemaining <= 5).length,
      overdue: pawnLoans.filter(l => l.status === 'Active' && (l.daysRemaining <= 0 || l.daysElapsed > 30)).length,
      redeemed: pawnLoans.filter(l => l.status === 'Redeemed').length
    };
  }, [pawnLoans]);
  ...
  const handleExecuteSettlement = () => {
    ...
    setTimeout(() => {
      if (settlementOption === 'redeem') {
        redeemLoan(selectedLoanForDrawer.ticketNumber, selectedLoanForDrawer.totalRedemptionAmount);
        showToast(
          'Loan Redeemed',
          `Ticket ${selectedLoanForDrawer.ticketNumber} settled. Asset released from ${selectedLoanForDrawer.vaultShelf}.`,
          'success'
        );
      } else {
        extendLoan(selectedLoanForDrawer.ticketNumber, selectedLoanForDrawer.extensionFee);
        showToast(
          'Term Extended',
          `Ticket ${selectedLoanForDrawer.ticketNumber} extended by +30 days under NCA.`,
          'success'
        );
      }
  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-y-auto bg-[#121212] p-3.5 sm:p-5 gap-4 relative">
      {/* ============================================================
          1. CLEAN SEARCH & FILTER SHELL
         ============================================================ */}
      <div className="space-y-3 bg-[#1E1E1E] rounded-xl p-3.5 sm:p-4 border border-[#2A2A2A] shadow-sm">
        ...
        <div className="flex items-center gap-2 overflow-x-auto no-scrollbar pt-1">
          <button
            ...
            className={`px-3.5 py-1.5 rounded-full text-xs font-semibold flex items-center gap-1.5 whitespace-nowrap transition ${
              filterTab === 'all'
                ...
                : 'bg-[#141414] border border-[#2A2A2A] text-gray-400 hover:text-gray-200'
            }`}
          >
          ...
          >
            ...
            <span className="px-1.5 py-0.2 rounded-full bg-black/30 text-amber-400 font-mono text-[10px]">
              {counts.expiring}
            </span>
          </button>
          ...
          >
            ...
            <span className="px-1.5 py-0.2 rounded-full bg-black/30 text-red-400 font-mono text-[10px]">
              {counts.overdue}
            </span>
          </button>
          ...
          >
            ...
            <span>Redeemed</span>
            <span className="px-1.5 py-0.2 rounded-full bg-black/30 text-emerald-400 font-mono text-[10px]">
              {counts.redeemed}
            </span>
          </button>
        </div>

        {/* Customer Pinned Filter Indicator */}
        {activeCustomer && (
          <div className="pt-2 border-t border-[#2A2A2A] flex items-center justify-between text-xs">
            <span className="text-gray-400">
              Filtering by active customer: <strong className="text-white font-headline">{activeCustomer.fullName}</strong>
            </span>
            <button
              type="button"
              onClick={() => setFilterOnlyActiveCustomer(prev => !prev)}
              className="text-[#E87A5D] hover:text-[#f0967d] font-mono text-[11px] underline"
            >
      ...
      {/* ============================================================
          2. STREAMLINED LOAN CARDS (2-COLUMN GRID)
         ============================================================ */}
      <div className="flex-1">
        {filteredLoans.length === 0 ? (
          <div className="p-10 text-center bg-[#1E1E1E] rounded-xl border border-dashed border-[#2A2A2A]">
            <Clock className="w-10 h-10 text-gray-500 mx-auto mb-2" />
            <p className="font-bold text-gray-300 text-sm">No Loan Records Match Filter</p>
            <p className="text-xs text-gray-500 mt-1">
              Adjust your search keywords or switch to "All Active".
            </p>
          </div>
        ) : (
            ...
            {filteredLoans.map(loan => {
              ...
              return (
                <div
                  ...
                  className="bg-[#1E1E1E] rounded-xl p-4 border border-[#2A2A2A] hover:border-[#C85A32] transition cursor-pointer flex flex-col justify-between gap-3 shadow-sm group"
                >
                  {/* Top Row: Ticket # + Days Remaining Pill */}
                  ...
                  {/* Customer Info & Relationship Actions */}
                  <div className="bg-[#141414] p-2.5 rounded-lg border border-[#2A2A2A] flex items-center justify-between group/cust">
                    ...
                    <button 
                      onClick={(e) => { e.stopPropagation(); showToast('WhatsApp', `Opening chat with ${loan.customerName}`, 'info'); }}
                  ...
                  {/* Bottom: Principal Loan + Open Drawer Trigger */}
      ...
      {/* ============================================================
          3. PROGRESSIVE SETTLEMENT DRAWER (SLIDE-OVER PANEL)
         ============================================================ */}
      {selectedLoanForDrawer && (
        <div className="fixed inset-0 z-50 overflow-hidden">
          {/* Softened Backdrop: background table remains visible */}
          <div
            onClick={() => setSelectedLoanForDrawer(null)}
          ...
          <div className="fixed inset-y-0 right-0 max-w-full flex pl-10">
            <div className="w-screen max-w-md bg-[#1E1E1E] border-l border-[#2A2A2A] shadow-2xl flex flex-col h-full animate-in slide-in-from-right duration-200">
              ...
              <div className="p-4 bg-[#161616] border-b border-[#2A2A2A] flex items-center justify-between shrink-0">
                <div>
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-bold text-[#E87A5D] font-mono">
                      {selectedLoanForDrawer.ticketNumber}
                    </span>
                    <span className="text-[11px] px-2 py-0.5 rounded-full bg-emerald-950 text-emerald-400 border border-emerald-800/40 font-mono">
                      Shelf: {selectedLoanForDrawer.vaultShelf}
                    </span>
                  </div>
                  <h3 className="text-sm font-bold text-white font-headline mt-1">
                    Counter Settlement Terminal
                  </h3>
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                {/* Customer & Collateral Summary Card */}
                <div className="p-3.5 bg-[#141414] rounded-xl border border-[#2A2A2A] space-y-2.5">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-xs font-bold text-white">{selectedLoanForDrawer.customerName}</p>
                      <p className="text-[11px] text-gray-400 font-mono mt-0.5">
                        SA ID: {selectedLoanForDrawer.customerIdNumber}
                      </p>
                      <p className="text-[11px] text-[#E87A5D] font-mono">
                        {selectedLoanForDrawer.customerMobile}
                      </p>
                    </div>
                    <div className="w-12 h-12 rounded-lg bg-[#1E1E1E] border border-[#2A2A2A] overflow-hidden shrink-0">
                      <img
                        src={selectedLoanForDrawer.itemImageUrl}
                        alt={selectedLoanForDrawer.itemTitle}
                        className="w-full h-full object-cover"
                      />
                    </div>
                  </div>

                  <div className="pt-2 border-t border-[#2A2A2A] flex items-center justify-between text-xs font-mono">
                    <span className="text-gray-400 truncate max-w-[220px]">
                      {selectedLoanForDrawer.itemTitle}
                    </span>
                    <span className="text-gray-300">SN: {selectedLoanForDrawer.serialOrImei}</span>
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-[10px] font-mono text-[#E87A5D]">Release Asset</span>
                      </div>
                      ...
                      <span className="text-[11px] font-mono text-gray-400 mt-0.5">
                        R {selectedLoanForDrawer.totalRedemptionAmount.toFixed(2)}
                      </span>
                    </button>
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-[10px] font-mono text-emerald-400">+30 Days</span>
                      </div>
                      <span className="text-xs font-bold mt-2 text-white">Extend Loan</span>
                      <span className="text-[11px] font-mono text-gray-400 mt-0.5">
                        R {selectedLoanForDrawer.extensionFee.toFixed(2)}
                      </span>
                    </button>
                  </div>
                </div>

                {/* SETTLEMENT OPTION A: FULL REDEMPTION BREAKDOWN */}
                {settlementOption === 'redeem' && (
                  <div className="bg-[#141414] rounded-xl p-3.5 border border-[#2A2A2A] space-y-2 text-xs font-mono">
                    <h4 className="text-xs font-bold text-white font-headline uppercase tracking-wider mb-2">
                      Full Redemption Breakdown
                    </h4>
                    <div className="flex justify-between text-gray-400">
                      <span>Principal Amount:</span>
                      <span className="text-white">R {selectedLoanForDrawer.principal.toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between text-gray-400">
                      <span>NCA Interest (5.00%/mo):</span>
                      <span className="text-[#E87A5D]">R {selectedLoanForDrawer.monthlyInterest.toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between text-gray-400">
                      <span>Storage &amp; Admin Fee:</span>
                      <span className="text-gray-300">
                        R {selectedLoanForDrawer.monthlyStorageAdminFee.toFixed(2)}
                      </span>
                    </div>
                    <div className="pt-2 border-t border-[#2A2A2A] flex justify-between items-baseline">
                      <span className="text-xs font-bold text-gray-300">Total Settlement Due:</span>
                ...
                {/* SETTLEMENT OPTION B: EXTEND 30 DAYS BREAKDOWN */}
                {settlementOption === 'extend' && (
                  <div className="bg-[#141414] rounded-xl p-3.5 border border-[#2A2A2A] space-y-2 text-xs font-mono">
                    <h4 className="text-xs font-bold text-white font-headline uppercase tracking-wider mb-2">
                      30-Day Extension Calculation
                    </h4>
                    <div className="flex justify-between text-gray-400">
                      <span>Monthly NCA Interest:</span>
                      <span className="text-[#E87A5D]">R {selectedLoanForDrawer.monthlyInterest.toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between text-gray-400">
                      <span>Vault Storage/Admin:</span>
                      <span className="text-gray-300">
                        R {selectedLoanForDrawer.monthlyStorageAdminFee.toFixed(2)}
                      </span>
                    </div>
                    <div className="pt-2 border-t border-[#2A2A2A] flex justify-between items-baseline">
                      <span className="text-xs font-bold text-gray-300">Extension Fee Due:</span>
                      <span className="text-lg font-bold text-emerald-400">
                        R {selectedLoanForDrawer.extensionFee.toFixed(2)}
                      </span>
                    </div>
                    <p className="text-[11px] text-gray-400 pt-1 font-sans">
                      Principal of R {selectedLoanForDrawer.principal.toFixed(2)} remains active in vault storage for another 30 days.
                    </p>
                  </div>
                )}
              ...
              <div className="p-4 bg-[#161616] border-t border-[#2A2A2A] space-y-2 shrink-0">
                <button
                  ...
                  className="w-full py-3 px-4 rounded-xl bg-[#C85A32] hover:bg-[#b04d29] text-white font-bold text-xs shadow-lg flex items-center justify-center gap-2 transition active:scale-[0.99] disabled:opacity-50"
                >
                  {isProcessing ? (
                    ...
                    <>
                      ...
                      <span>Receive Payment &amp; Release Asset from Vault</span>
                    </>
                  ) : (
                    <>
                      <RotateCcw className="w-4 h-4" />
                      <span>Collect Fee &amp; Extend Maturity Date</span>
                    </>
                  )}
                </button>
                ...
                <button
                  type="button"
                  onClick={() => setSelectedLoanForDrawer(null)}
                  className="w-full py-2 text-center text-xs text-gray-400 hover:text-white transition"
                >
                  Cancel &amp; Close Drawer
                </button>
              </div>
