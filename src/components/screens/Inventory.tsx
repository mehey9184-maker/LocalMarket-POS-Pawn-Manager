...
export const Inventory: React.FC = () => {
  ...
  // Reset display window limit when tab, acquisition filter, or search changes
  React.useEffect(() => {
    setDisplayLimit(48);
  }, [tab, acquisitionFilter, search]);
            ...
            {filteredItems.length > displayLimit && (
              <div className="pt-4 pb-2 text-center">
                <button
                  ...
                  className="px-6 py-2.5 rounded-xl border border-gray-300 bg-white hover:bg-gray-50 text-xs font-bold text-gray-700 transition shadow-xs cursor-pointer"
                >
                  Show More Catalog Items ({filteredItems.length - displayLimit} remaining)
                </button>
              </div>
            )}
          </div>
