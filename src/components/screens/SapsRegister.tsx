    ...
    <div className={`flex-1 flex flex-col h-full bg-[#F5F6F8] p-4 sm:p-6 gap-5 overflow-hidden transition-colors ${isPoliceInspectionMode ? 'border-2 border-blue-500 rounded-2xl' : ''}`}>
      ...
      {recordedVisit && isPoliceInspectionMode && (
        <div className="bg-blue-50 border border-blue-200 rounded-xl px-4 py-2.5 flex items-center justify-between text-[11px] font-mono text-blue-900 shrink-0">
          <div className="flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-emerald-600" /><span>Inspection Logged (Merchant Entry): <strong className="text-gray-900">{recordedVisit.rank} {recordedVisit.officerName}</strong> • {recordedVisit.timestamp}</span></div>
          <span className="px-2.5 py-0.5 rounded-full bg-emerald-100 text-emerald-800 border border-emerald-300 font-bold text-[10px] uppercase">Merchant Logged</span>
        </div>
      )}
