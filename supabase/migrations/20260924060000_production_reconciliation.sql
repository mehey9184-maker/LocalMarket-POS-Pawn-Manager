...
-- Remove dangerous default '1234' from pin_code for new accounts
ALTER TABLE IF EXISTS public.profiles 
    ALTER COLUMN pin_code DROP DEFAULT;

-- 2. STAFF AUDIT LOGS TABLE
...
-- 4. SELLER TRANSACTIONS & REVERSALS
...
-- 5. OWNER BUSINESS RULES & AUDIT LOGS
...
CREATE POLICY "Shop owners/managers access business rule audit logs" ON public.business_rule_audit_logs
    ...
    USING (
        shop_id IN (
            SELECT shop_id FROM public.profiles WHERE id = auth.uid() AND role IN ('owner', 'admin', 'manager')
        )
    );

-- 6. ATOMIC BRUTE-FORCE LOCKOUT FUNCTIONS
...
-- 7. SECURE STAFF PROFILE UPDATE RPC
...
-- 8. HARDEN SEARCH_PATH ON EXISTING FUNCTIONS
...
CREATE POLICY "Authenticated users view shop profiles" ON public.profiles
    FOR SELECT TO authenticated
    USING (
        shop_id IN (
            SELECT shop_id FROM public.profiles WHERE id = auth.uid()
...
CREATE POLICY "Users update own profile or managers update shop profiles" ON public.profiles
